...
time.sleep(5)   
# Search for XLSX files in the directory
xlsx_files = glob.glob(os.path.join(directory_path, '*.xlsx'))

# Sort the XLSX files by their download date in descending order
xlsx_files.sort(key=lambda x: os.path.getctime(x), reverse=True)

# Check if there are any XLSX files in the directory
if len(xlsx_files) > 0:
    # Get the most recently downloaded XLSX file
...
#Extract my list of learners from the vle
#learner_list_dict=a.get_learner_list()
#name,url=select_learner(learner_list_dict)
#learner_name=name
#Extract the learner ID from the url https://vle.tdm.co.uk/totara/evidence/view.php?id=108
#id=url.split('?id=')[1]
#print(f"id={id}")

#Create the new download directory for this learner and set teh report name